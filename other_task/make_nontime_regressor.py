...
# 회귀 인터페이스
with gr.Blocks(title="예측 모델을 만듭니다.", theme=gr.themes.Soft(font=fonts.GoogleFont("Jua")),css="temp/style.css") as non_time_regression:
    gr.Markdown(
        """
        # 지금부터 예측 모델 만드는 도움을 드릴게요
        """
        , elem_id='title'
    )
    task = "Regressor"
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            text4=gr.Text(label="질문 4", value="오른쪽에서 범주형이라고 출력된 수는 데이터에서 시간정보가 기록된 열을 의미합니다. 시간정보로 기록된 열을 수치형으로 바꿔주는 과정을 수행하면 모델의 성능 향상에 도움이 될 수 있습니다. 진행할까요?")
            time_decom = gr.Radio(label="선택", choices=["예", "아니오"])
            decom_btn = gr.Button(value="작업이 실행됩니다")
            # phase 2 : 데이터 삭제부터 split까지
            text5 = gr.Text(label="질문 5", value="데이터에서 필요없는 정보가 있나요? 있다면 선택해주세요 만약 시간과 관련된 정보가 있다면 제거해주세요", visible=False)
